...
def aggregate(ground_truth_folder, edges_path, dataset):
    ...
    if dataset == "trace" or dataset == "theia":
        firefox_backdoor = pd.read_csv(os.path.join(ground_truth_folder, f"TC3_{dataset}_firefox_backdoor.csv"))
        browser_extension = pd.read_csv(os.path.join(ground_truth_folder, f"TC3_{dataset}_browser_extension.csv"))
        if dataset == "trace":
            pine_phishing_exe = pd.read_csv(os.path.join(ground_truth_folder, f"TC3_{dataset}_pine_phishing_exe.csv"))
            trace_thunderbird_phishing_exe = pd.read_csv(os.path.join(ground_truth_folder, f"TC3_{dataset}_thunderbird_phishing_exe.csv"))
...
if __name__=="__main__":
    ...
    print(args)
    aggregate(args.ground_truth_folder, args.edges_path, args.dataset)